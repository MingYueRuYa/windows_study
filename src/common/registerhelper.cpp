...
DWORD RegisterHelper::IsExsitKey(const wstring &keyName) const
{
    wstring subpath = mSubPath + (keyName.empty() ? wstring(L"") : (wstring(L"\\") + keyName));
...
DWORD RegisterHelper::CreateKey(const wstring &keyName)
{
    wstring subpath = mSubPath + (keyName.empty() ? wstring(L"") : (wstring(L"\\") + keyName));
    HKEY hKey;
    DWORD errorcode = RegOpenKeyExW(mRootKey, subpath.c_str(), 0, 
											mSamDesired, &hKey);
    if (ERROR_SUCCESS != errorcode) { 
        errorcode = RegCreateKey(mRootKey, subpath.c_str(), &hKey);
